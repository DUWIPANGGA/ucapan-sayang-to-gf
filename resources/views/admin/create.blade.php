...
@section('title', 'Tambah Valentine')

@section('content')
<div class="card">
    <div class="card-header">
        <h1 class="card-title">Tambah Valentine Baru</h1>
    ...
    <form action="{{ route('admin.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="text" name="name" class="form-input" value="{{ old('name') }}" required placeholder="Contoh: Cilla">
            <div class="form-hint">Link akan menjadi /{{ Str::slug(old('name', 'nama')) }}</div>
        </div>
        ...
        <div class="form-group">
            ...
            <input type="text" name="ucapan" class="form-input" value="{{ old('ucapan') }}" placeholder="Contoh: Happy Valentine!">
            <div class="form-hint">Ucapan yang muncul di typewriter</div>
        </div>

        <div class="form-group">
            <label class="form-label">Pesan / Description</label>
            <textarea name="message" class="form-input" placeholder="Tulis pesan spesial untuk dia...">{{ old('message') }}</textarea>
